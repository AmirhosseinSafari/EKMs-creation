...
def little_ekm_sbf_bpf_dataset_shifted(lead_data, sampling_rate, dataset_name, ekms_path, key, sbf, shift_amount):
    ...
    while(ekms_counter<total_ecms):
      ...
      ecm = electrocardiomatrix_sbf_bpf(distance, peaks, filtered_ecg, ekms_counter, sampling_rate)
...
def user_ekm_sbf_bpf_shifted_dataset(ecg_file, shared_counter_, lock, total_elements, shifted_amount):
    # print(f"\n{ecg_file}")
    ...
    for _, lead_data in enumerate(user_leads_signals):
        ...
        little_ekm_sbf_bpf_dataset_shifted(lead_data.data, sampling_rate, dataset_name, lead_path, user_id, sbf, shifted_amount)
